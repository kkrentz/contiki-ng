...
- There is a performance break point when there are more than 17 nodes in a network. In such networks, `nbr-table` has to decide which neighbors to keep and which not. As AKES currently locks all entries in `akes-nbr.c`, we need to override this behavior. A smart policy should, on the one hand, avoid isolated clusters and, on the other hand, take existing information about link qualities into account.

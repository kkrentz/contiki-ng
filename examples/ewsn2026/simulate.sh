  ...
  do
    ...
    do
      ...
      java -jar ../../../../tools/cooja/build/libs/cooja.jar \
           simulation.csc --no-gui --random-seed=${run} \
           && tail -n +2 COOJA.testlog | head -n -2 > ../../statistics/$run/$name.csv \
           && echo "t;name;type;address;rx;tx;total" > ../../statistics/$run/$name-energy.csv \
           && sed '/energy/!d' ../../statistics/$run/$name.csv >> ../../statistics/$run/$name-energy.csv \
           && sed -i '/energy/d' ../../statistics/$run/$name.csv &
      sleep 1
      popd
    done
    wait

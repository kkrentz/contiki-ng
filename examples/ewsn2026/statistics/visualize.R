...
# set colors
rate_colors <- c("#AE003A", "#FF7221", "#FFDD00", "#4BD697", "#1A754C")
...
# determine ids
data <- read.table(paste0("1/", experiments[1], "-", rates[1], ".csv"), header = T, sep = ";")
...
for (rate_index in 1:length(rates)) {
  for (experiment_index in 1:length(experiments)) {
    run_means <- c()
    for (run in runs) {
      data <- read.table(paste0(run, "/", experiments[experiment_index], "-", rates[rate_index], ".csv"),
                         header = T,
                         sep = ";")
      ...
      # collect the mean over the mean per-node delivery ratios
      run_means <- c(run_means, mean(per_node_delivery_ratios) * 100)
      # generate a figure for each individual run
      svglite(file = paste0(run, "/", experiments[experiment_index], "-", rates[rate_index], ".svg"),
              width = 12,
              height = 6)
      barplot(per_node_delivery_ratios)
      dev.off()
    }
    delivery_ratios[rate_index, experiment_index] <- mean(run_means)
    standard_error <- sd(run_means) / sqrt(length(run_means))
    confidences[rate_index, experiment_index] <- t_value * standard_error
  }
}
...
par(cex = 1)
par(las = 1)
par(mar = c(5.1-1.1, 4.1+0.9, 4.1-4.1, 2.1-2.1), family = "Aptos")
...
for(i in seq(length(delivery_ratios))) {
  text(delivery_ratios[i] + 1,
       centers[i],
       paste0("\u2300 ",
              format(round(delivery_ratios[i], 2), nsmall = 2),
              "%",
              " \u00b1 ",
              format(round(confidences[i], digits = 2), nsmall = 2),
              "%"), adj = c(0,0.4), cex=0.9)
}
legend("topright",
       rev(txrates),
...
for (experiment_index in 1:length(experiments)) {
  for (rate_index in 1:length(rates)) {
    ...
    for (run in runs) {
      ...
      for (id in ids) {
        received_datagrams <- subset(data, (event == "r") & (address_to_id(address) == id))
        # remove duplicate receptions (may happen when using SMOR)
        ...
        for (c in received_datagrams$counter) {
          ...
          per_run_delays <- c(per_run_delays, delay)
          all_delays <- c(all_delays, delay)
        }
      }
      run_means <- c(run_means, mean(per_run_delays))
    }
    i <- i + 1
    delays[[i]] <- all_delays
    maxima <- c(maxima, max(all_delays))
    means <- c(means, mean(run_means))
    standard_error <- sd(run_means) / sqrt(length(run_means))
    confidences[rate_index, experiment_index] <- t_value * standard_error
  }
}
...
for (at_vector_index in 1:length(at_vector)) {
  text(maxima[at_vector_index] + 0.8,
       at_vector[at_vector_index],
       paste("\u2300",
             format(round(means[at_vector_index], 2), nsmall = 2),
             "s",
             "\u00b1",
...
for (experiment_index in 1:length(experiments)) {
  for (rate_index in 1:length(rates)) {
    ...
    tx_run_means <- c()
    for (run in runs) {
      data <- read.table(paste0(run, "/", experiments[experiment_index], "-", rates[rate_index], "-energy.csv"),
                         header = T,
                         sep = ";")
      rx_percentages <- c()
      tx_percentages <- c()
      ...
      rx_run_means <- c(rx_run_means, mean(rx_percentages))
      tx_run_means <- c(tx_run_means, mean(tx_percentages))
    }
    i <- i + 1
    e[1, i] <- mean(rx_run_means)
    e[2, i] <- mean(tx_run_means)
    duty_cycles <- rx_run_means + tx_run_means
    standard_error <- sd(duty_cycles) / sqrt(length(duty_cycles))
    confidences[rate_index, experiment_index] <- t_value * standard_error
  }
}
...
axis(1, cex.axis = 1, at = seq(0,7,1))
i <- 0
for (experiment_index in 1:length(experiments)) {
  for (rate_index in 1:length(rates)) {
    i <- i + 1
    text(e[1, i] + e[2, i] + 0.1,
         centers[i],
         paste0("\u2300 ",
                format(round(e[1, i] + e[2, i], 2), nsmall = 2),
                "%",
                " \u00b1 ",
                format(round(confidences[i], digits = 2), nsmall = 2), "%"), adj = c(0,0.5))
  }
}
mids <-c()
i <- 0
...
if ((length(experiments) == 4) && (experiments[2] == "csl-classic-ml") && (experiments[4] == "orchestra")) {
  hpi_mac_indices <- 1:length(rates) + length(rates) * 0
  ...
  for(rate_index in 1:length(rates)) {
    ...
    duty_cycle_orchestra <- e_orchestra[1,rate_index] + e_orchestra[2,rate_index]
